...
- **Turbo network (built-in Tailscale)** — each app runs its own Tailscale node in user space (`turbo-net`, the official `tsnet` library; no install, no daemon, no root, no system extension). The join key comes from turbolink at launch, so nothing is typed: the Receiver's header shows its 100.x address and the pairing announces it; the Streamer sees a 100.x destination and tunnels the SRT through its node (packets sized for the tunnel's 1280 MTU). The log says whether the path is direct or through a Tailscale relay. Until the server has a Tailscale API token, the app shows a one-time login link instead
