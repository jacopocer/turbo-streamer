...
- **Versioning + self-update**: `VERSION` (marketing version) + `git rev-list --count HEAD` (build) are stamped into both Info.plists by build.sh (placeholders `__SHORT__`/`__BUILD__` filled by sed). `Updater.swift` (both apps) fetches `GET /v1/appcast` on turbolink, compares `build` to the running `CFBundleVersion`, and on a newer build downloads the zip, verifies its sha256, extracts with ditto, then a detached script swaps it into `/Applications` (ditto to `<App>.app.new`, remove old, `mv` into place — atomic, no rsync quick-check skips) and relaunches. The version shows in each app's footer (click = check now); a newer build shows a banner with Update & Relaunch. Publish with `link-server/deploy/deploy-apps.sh` (owner-run): builds both, `ditto`-zips them, scp to `/opt/turbolink/downloads`, writes `appcast.json`. turbolink serves `/v1/appcast` and `/downloads/<file>.zip` (path-sanitised, zip-only); nginx is not touched — files stream through Node.
- **First-run wizard** (`RootView.swift` → `OnboardingView`, both apps): shown once (`<App>.didOnboard` in UserDefaults), it explains the app and walks the operator onto the Turbo network, naming the reference account **garibaldi@indigital.tv** to sign in with on every machine. Connect / Open login page / Done, plus Skip.
- **Turbo network (`net/turbo-net`, Go + tsnet; `TurboNet.swift` in both apps)**: one helper process per app = one Tailscale node in user space, state in Application Support/<App>/tailnet, hostname `turbo-<app>-<machine>`. Commands over stdin (`listen`/`dial`/`close`, JSON per line), events over stdout (`auth_url`, `up`, `listening`, `local`, `peer`, `closed`, `error`). The Receiver runs `listen srt 8890 → 127.0.0.1:8890` when MediaMTX starts and its `selectedAddress` jumps to the 100.x once the node is up; the Streamer, in pre-flight, sees a 100.64/10 destination and `dial`s it, pointing ffmpeg at the returned local port (`destinationOverride`, `tailnetRuns` → `-pkt_size 1200`). Join key: `POST /v1/tailnet/key` on turbolink mints a single-use, ephemeral, pre-authorised, tagged key through the Tailscale API (`TS_API_TOKEN` in `/opt/turbolink/tailnet.env`, read by the unit's `EnvironmentFile`); with a key the helper runs `--ephemeral`, without one it prints a login URL (once per app, state then persists). Rebuild the helper with `net/build-net.sh` (needs Go); both build.sh bundle `net/bin/turbo-net` and warn if it's missing. **Not yet exercised end to end**: it needs the owner's tailnet (token on the server, or one login per app).
- **Relay (option B, `link-server/deploy/turborelay.*`)**: MediaMTX v1.21.0 next to turbolink on the prod box, SRT UDP 8890 + RTMP TCP 1935, API on loopback 9997, MoQ off. Every session now carries `relay.publishPass`/`readPass`; MediaMTX authorises each publish/read by POSTing to turbolink's `/v1/auth` over localhost (refused when `X-Real-IP`/`X-Forwarded-For` is present, i.e. anything that came through nginx). The path is the session code. The streamer's "Use relay" fills url/key with `srt://host:8890` + `CODE:streamer:<pass>` and stores the RTMP door in `altRTMPURL`/`altStreamKey`; each receiver's Link stores `relaySource` (`srt://host:8890?streamid=read:CODE:receiver:<pass>`) and the feed card gets a Direct/Relay switch that sets the MediaMTX path `source` (live via `PATCH /v3/config/paths/patch`). Sessions expire 24 h after their last use (poll, join, auth), not after creation. Verified locally end to end (publish, pull as path source, read, wrong secret → 401); the box deploy is `deploy-relay.sh`, owner-run.
- **Automatic transport ladder (direct → relay-SRT → relay-RTMP)**: "Use (auto)" in the streamer's Link popover stores the receiver's direct SRT, the relay SRT and RTMP doors, and the code+secret, and sets `autoPair`. At start, `resolveAutoTransport` probes them best-first (tunnelling a 100.x through turbo-net), picks the first that works, sets `destinationOverride`, badges the Live card, and reports the choice to turbolink (`POST /v1/session/:code/transport`, secret-gated). The receiver polls `GET /v1/session/:code/transport` (code only) via `startTransportFollow` and applies the matching source: `publisher` for direct, the relay SRT or RTMP URL for the relay — so both ends align automatically. Manual "Direct only" / the receiver's Direct/Relay toggle remain for feeds without a code. HEVC is clamped to H.264 on any non-SRT output (flv rejects HEVC). Verified: turbolink transport endpoints (report gated by secret, poll by code); the transport pieces (SRT probe, tunnel, RTMP relay pull) each end-to-end earlier.
- **SRT pre-flight (`probeSRT`)**: connects with a deliberately wrong streamid; "rejected" within ~1 s means the UDP path is open, the 3 s timeout means blocked or closed. When blocked and `altRTMPURL` is set, the run uses the relay's RTMP door (`destinationOverride[id]`, threaded into `buildArgs` and the slate). Nothing is ever published by the probe.
- **Network passthrough** (`networkPassthrough`): `-map 0:v:0 -map 0:a:0? -c copy`, no filters or encoder; overlay/scaling/fps/mute/adaptive/detectors are out, backup and recording (tee) still work. Mute logs and returns. An `srt://…?streamid=…` network source is rewritten to `-srt_streamid` (ffmpeg ignores the query form) in `networkInputArgs`, used by the stream and the preview alike.
- **Encoder-behind diagnostic**: `StreamStatus.slowSamples` counts consecutive progress lines with speed < 0.95x; six in a row (~3 s) raise `Diagnostic.encoderBehind` on the Live card. Measured, not matched, so it works on any Mac regardless of the Auto thresholds.
...
- The fallback slate publishes through the same `primaryOutputArgs` as the stream, so it works on SRT destinations too (it used to be FLV-only).
